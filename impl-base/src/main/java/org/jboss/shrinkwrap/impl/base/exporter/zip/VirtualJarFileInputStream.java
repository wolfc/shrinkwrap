...
import static org.jboss.shrinkwrap.impl.base.exporter.zip.VirtualJarInputStream.CENHDR;
import static org.jboss.shrinkwrap.impl.base.exporter.zip.VirtualJarInputStream.javaToDosTime;

/**
 * An input stream that can be used to wrap an VirtualJarInputStream (so any Archive)
...
class VirtualJarFileInputStream extends InputStream
{
   ...
   private enum State
   {
      ...
      ENTRY_CONTENT
              {
                 ...
                 @Override
                 int read(final VirtualJarFileInputStream jarFileInputStream, final byte b[], final int off, final int len) throws IOException
                 {
                    final InputStream in = jarFileInputStream.currentInputStream;
      ...
      int read(VirtualJarFileInputStream jarFileInputStream, byte b[], int off, int len) throws IOException
      {
